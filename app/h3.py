...
def _insert_enrichment_after_first(value: str, pattern: str, detail: str) -> str:
    match = re.search(pattern, value, flags=re.I)
    if not match:
        return value
    return f"{value[:match.end()]}{detail}{value[match.end():]}"


def bounded_enrich_prompt(source: str, strength: int) -> str:
    """Add only deterministic production guidance at source-supported cue points."""
    ...
    enriched = value
    if strength >= 30:
        enriched = _insert_enrichment_after_first(
            enriched, r"\u7279\u5199[^,\uff0c\u3002\uff1b;\n]*", "\uff0c\u7126\u70b9\u7a33\u5b9a\u843d\u5728\u6309\u952e\u548c\u624b\u90e8\u52a8\u4f5c\u4e0a"
        )
    if strength >= 50:
        enriched = _insert_enrichment_after_first(
            enriched, r"\u5207\u6362(?:\u5230)?(?:\u4e2d|\u8fd1|\u8fdc|\u5168)\u666f", "\uff0c\u4eba\u7269\u76f8\u5bf9\u4f4d\u7f6e\u4e0e\u524d\u540e\u52a8\u4f5c\u4fdd\u6301\u8fde\u7eed"
        )
    if strength >= 80:
        enriched = _insert_enrichment_after_first(
            enriched, r"\u9759\u6b62\u4e0d\u52a8", "\uff0c\u9759\u6b62\u59ff\u6001\u4e0e\u7a7a\u6d1e\u76ee\u5149\u5728\u955c\u5934\u5207\u6362\u540e\u4fdd\u6301\u4e00\u81f4"
        )
        enriched = _insert_enrichment_after_first(
            enriched, r"\u6781\u4f4e\u89c6\u89d2\u4ef0\u62cd", "\uff0c\u955c\u5934\u65b9\u5411\u4e0e\u4eba\u7269\u7a7a\u95f4\u5173\u7cfb\u4fdd\u6301\u6e05\u6670"
        )
    if strength >= 100:
        enriched = _insert_enrichment_after_first(
            enriched, r"\u9065\u63a7\u5668", "\uff0c\u6309\u952e\u52a8\u4f5c\u7684\u8282\u594f\u4e0e\u540e\u7eed\u955c\u5934\u5207\u6362\u4fdd\u6301\u8fde\u8d2f"
        )
    if enriched == value:
        suffix = "\uff0c\u955c\u5934\u4fdd\u6301\u65e2\u6709\u52a8\u4f5c\u548c\u7a7a\u95f4\u5173\u7cfb\u8fde\u7eed\u3002" if _contains_cjk_text(value) else " The camera maintains the existing action and spatial continuity."
        enriched = f"{value.rstrip('。.')}{suffix}"
    return enriched
...
def scene_enrichment_system(strength: int) -> str:
    strength = max(60, min(100, int(strength)))
    detail = "one concise scene and lighting clause" if strength < 80 else "one or two concise scene, lighting, and atmosphere clauses" if strength < 100 else "two or three vivid but static scene, lighting, and atmosphere clauses"
    return f"""Write only {detail} for the supplied video prompt in the same language as the source. Creative strength is {strength}/100. You may introduce a new location, time of day, weather, lighting, background texture, or atmosphere. Do not mention any person, gender, body part, clothing, prop, dialogue, relationship, intention, action, event, camera movement, shot, transition, or plot. Return only the scene atmosphere text, without headings, labels, markdown, or explanations."""


def scene_enrichment_token_limit(strength: int) -> int:
    return 96 if strength < 80 else 160 if strength < 100 else 224


def is_safe_scene_enrichment(value: str) -> bool:
    candidate = str(value or "").strip()
    if not candidate or len(candidate) > 360:
        return False
    forbidden = (
        r"\u7537|\u5973|\u4ed6|\u5979|\u4eba|\u8eab\u5f71|\u4eba\u5f71|\u526a\u5f71|\u8eab\u4f53|\u8863|\u88d9|\u8138|\u624b|\u817f|\u5934|"
        r"\u51fa\u73b0|\u8d70|\u8dd1|\u62ff|\u6309|\u62cd|\u6478|\u8e72|\u62b1|\u8e6d|\u543b|\u8bf4|\u558a|\u5bf9\u8bdd|\u4f20\u6765|"
        r"\u955c\u5934|\u7279\u5199|\u5207\u6362|\u52a8\u4f5c|\u4e8b\u4ef6|\u5267\u60c5|"
        r"\u9065\u63a7\u5668|\u7535\u89c6|\u51b0\u7bb1|\u53f0\u706f|\u540a\u706f|\u8def\u706f|\u706f\u5177|\u7a97|\u95e8|\u684c|\u6905|\u5e8a|\u624b\u673a|\u6b66\u5668|\u4f1e|\u5305|\u676f|\u5200|\u67aa|\u5c4f\u5e55|\u6c7d\u8f66|\u8f66\u8f86|"
        r"\b(?:man|woman|person|people|human|body|face|hand|leg|head|clothing|dress|shirt|"
        r"walk(?:s|ed|ing)?|run(?:s|ning)?|hold(?:s|ing)?|press(?:es|ed|ing)?|speak(?:s|ing)?|"
        r"say(?:s|ing)?|camera|shot|close[- ]up|cut|transition|dialogue|plot|event|"
        r"remote|television|tv|refrigerator|fridge|blind|curtain|door|table|chair|bed|phone|"
        r"weapon|car|vehicle|umbrella|bag|cup|knife|gun|screen)\b"
    )
    return not re.search(forbidden, candidate)


def fallback_scene_enrichment(strength: int, use_cjk: bool = True) -> str:
    if not use_cjk:
        if strength >= 100:
            return "A deserted industrial space at dusk combines uneven shadow planes, cool blue daylight, mist, wet ground, and stark contrast into a surreal yet restrained atmosphere."
        if strength >= 80:
            return "An empty street on a rainy night is layered with wet reflections, deep shadows, and cool light for a restrained, tense atmosphere."
        return "A dim indoor setting is held in cool moonlight and still air."
    if strength >= 100:
        return "\u66ae\u8272\u4e2d\u7684\u8352\u5e9f\u5de5\u4e1a\u7a7a\u95f4\u5ef6\u5c55\u51fa\u9ad8\u4f4e\u9519\u843d\u7684\u6697\u9762\u4e0e\u51b7\u84dd\u5929\u5149\uff0c\u8584\u96fe\u3001\u6e7f\u6da6\u5730\u9762\u548c\u5f3a\u70c8\u660e\u6697\u5bf9\u6bd4\u5f62\u6210\u8d85\u73b0\u5b9e\u800c\u514b\u5236\u7684\u6c1b\u56f4\u3002"
    if strength >= 80:
        return "\u96e8\u591c\u7684\u7a7a\u65f7\u8857\u9053\u8986\u76d6\u7740\u6f6e\u6e7f\u53cd\u5149\u4e0e\u6df1\u9083\u9634\u5f71\uff0c\u51b7\u8272\u5149\u7ebf\u8425\u9020\u51fa\u514b\u5236\u800c\u7d27\u5f20\u7684\u6c1b\u56f4\u3002"
    return "\u660f\u6697\u7684\u5ba4\u5185\u573a\u666f\u7b3c\u7f69\u5728\u51b7\u8272\u6708\u5149\u4e0e\u5bc2\u9759\u7a7a\u6c14\u4e2d\u3002"


def integrate_scene_enrichment(prompt: str, scene: str) -> str:
    value = str(prompt or "").strip()
    detail = str(scene or "").strip().strip("\uff0c,\u3002\uff1b; ")
    if not value or not detail:
        return value
    start_reference = re.search(r"\u89c6\u9891(?:\u573a\u666f)?(?:\u662f)?(?:\u5f00\u59cb\u4e8e|\u4ece)\s*\u56fe\s*\d+", value)
    insertion = f"\uff0c\u573a\u666f\u6c1b\u56f4\uff1a{detail}"
    if start_reference:
        return f"{value[:start_reference.end()]}{insertion}{value[start_reference.end():]}"
    return f"\u573a\u666f\u6c1b\u56f4\uff1a{detail}\uff0c{value}"
