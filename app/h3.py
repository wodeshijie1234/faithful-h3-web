...
def _ref2va_action_sentences(translation: str) -> list[str]:
    ...
    value = re.sub(
        r"<Picture\s+\d+>\s+is\s+(?:(?:a|the)\s+)?(?:man|male|boy|woman|female|girl)"
        r"(?:\s+(?:reference\s+)?(?:image|picture))?\s*[,.;]?\s*",
        "",
        value,
        flags=re.I,
    )
...
def enrichment_system(strength: int, target_length: int = 500) -> str:
    ...
    if strength <= 30:
        creativity = "Add only minimal framing, focus, continuity, physically continuous motion, and directly supported ambience. Do not introduce new story facts."
    elif strength <= 60:
        creativity = "You may introduce compatible lighting, atmosphere, environmental texture, camera movement, sound, and small physical reactions, but no new character, relationship, dialogue, or plot turn."
    elif strength <= 80:
        creativity = "You may introduce richer compatible setting, lighting, atmosphere, camera design, sound, gestures, and minor props that support the existing action, but no new character, relationship, dialogue, or independent plot event."
    else:
        creativity = "You may richly develop compatible setting, time of day, weather, lighting, wardrobe detail, minor props, gestures, camera progression, and sound design, but only while the supplied actions are occurring. Do not add any new event, destination, message, relationship, task, encounter, or aftermath. Creative additions are expected, but they must not contradict, replace, or remove the supplied people, identities, counts, relationships, actions, order, references, or dialogue. Do not add new characters or change the core premise."
    return f"""Return one complete enriched video prompt, not additions and not an afterword. Rewrite the complete source once while integrating each new detail beside the exact source action it develops. Do not prepend, append, quote, or repeat the unchanged source, and do not restart the action sequence. Target output length is {target_length} characters; the finished result must contain between {lower_bound} and {upper_bound} non-whitespace characters when the source is not already longer. Never truncate, omit, contradict, or sanitize source facts merely to meet the target. Keep every source identity, reference ID, count, position, action, camera change, sequence, continuity condition, vocalization, and dialogue in the original order. Write in exactly the same language as the input; Chinese input must produce Chinese output. Creative strength is {strength}/100. {creativity} All added detail must occur during an existing source action; enrich that action's visible execution, camera, light, environment, physical continuity, timing, and supported sound. Once the final source action, vocalization, or dialogue is complete, stop immediately. Never describe an aftermath or continue the story beyond the source ending. Do not change the nature of the source activity, its genre or emotional tone. Unless already explicit in the source, never invent violence, harm, coercion, sexual content, crime, disease, or injury. Never invent biography such as a person's name, age, occupation, history, body measurements, or medical condition. Preserve every supplied utterance verbatim and exactly once; never invent an additional spoken line merely because the source already contains dialogue. These restrictions apply only to new inventions: preserve any such fact that the user explicitly supplied. Integrate all additions into one cohesive production-ready prompt instead of repeating the source and appending an afterword. Use the requested length for concrete, filmable development within the supplied action chain rather than repetition, aftermath, a sequel, or generic filler. No headings, labels, markdown, explanations, or H3 fields."""
...
def enrichment_repair_system(strength: int, target_length: int = 500) -> str:
    ...
    upper_bound = round(target_length * 1.1)
    if strength <= 30:
        creativity = "Retain only minimal framing, continuity, motion, and supported ambience additions."
    elif strength <= 60:
        creativity = "Retain compatible lighting, atmosphere, environmental texture, camera, sound, and small reaction details; remove new characters, relationships, dialogue, and plot turns."
    elif strength <= 80:
        creativity = "Retain compatible setting, lighting, atmosphere, camera, sound, gestures, and supporting minor props; remove new characters, relationships, dialogue, and independent plot events."
    else:
        creativity = "Retain or add compatible setting, time, weather, lighting, wardrobe detail, minor props, gestures, camera progression, and sound design only while supplied actions are occurring. Remove every new event, destination, message, relationship, task, encounter, or aftermath, as well as contradictions, replacement of the core premise, new characters, or changes to supplied identities, counts, relationships, actions, order, references, and dialogue."
    return f"""Repair the PROPOSED ENRICHMENT against the ORIGINAL SOURCE. Return one complete enriched prompt in exactly the same language as the ORIGINAL SOURCE; Chinese source must produce Chinese output. Rewrite the complete source once while integrating each retained addition beside its corresponding source action. Do not prepend, append, quote, or repeat the unchanged source, and do not restart the action sequence. Target output length is {target_length} characters; the finished result must contain between {lower_bound} and {upper_bound} non-whitespace characters when the source is not already longer. Preserve every explicit source identity, reference ID, count, position, action, camera direction, sequence, continuity statement, vocalization, and dialogue in the original order. Creative strength is {strength}/100. {creativity} All added detail must occur during an existing source action. Once the final source action, vocalization, or dialogue is complete, stop immediately; remove all aftermath and further story. Do not change the nature of the source activity, its genre or emotional tone. Remove invented violence, harm, coercion, sexual content, crime, disease, or injury unless it is explicit in the source. Remove invented biography, including names, ages, occupations, histories, body measurements, and medical conditions. Preserve every supplied utterance verbatim and exactly once, and remove every other spoken line. Never remove or sanitize explicit source facts. Integrate additions into the prompt itself, remove source-plus-afterword formatting, repetition, sequels, and generic filler, and never replace a source action with a different action. Return only the repaired prompt, without headings, labels, markdown, or explanations."""


def enrichment_continuation_system(strength: int, remaining_length: int) -> str:
    strength = max(1, min(100, int(strength)))
    remaining_length = max(100, min(700, int(remaining_length)))
    if strength <= 30:
        allowed = "Continue only framing, focus, physical continuity, and directly supported ambience."
    elif strength <= 60:
        allowed = "You may continue compatible lighting, atmosphere, environmental texture, camera movement, sound, and small physical reactions."
    elif strength <= 80:
        allowed = "You may continue compatible setting, lighting, atmosphere, camera design, sound, gestures, and supporting minor props."
    else:
        allowed = "You may continue compatible setting, time, weather, lighting, wardrobe detail, minor props, gestures, camera progression, and sound design only within the supplied action currently being described. Do not add an event or aftermath."
    lower_bound = max(80, round(remaining_length * 0.8))
    upper_bound = min(750, max(lower_bound, round(remaining_length * 1.15)))
    return f"""Write only a new continuation that can be appended directly to the existing enriched video prompt. Do not repeat, summarize, restart, quote, or rewrite the existing prompt. Use exactly the same language as the source. Creative strength is {strength}/100. {allowed} Preserve the existing people, identities, counts, relationships, core action, references, sequence, and dialogue. Do not change the nature of the source activity, its genre or emotional tone. Unless already explicit in the source, do not add violence, harm, coercion, sexual content, crime, disease, injury, biography, names, ages, occupations, histories, body measurements, medical conditions, dialogue, or a new principal character. Write no direct speech, indirect speech, quoted or unquoted utterance, spoken question, spoken exclamation, or attribution such as says, asks, answers, shouts, or explains unless the source explicitly requests dialogue. Describe only visible staging, safe neutral actions, physical continuity, environment, lighting, camera progression, and non-vocal sound. Never remove or sanitize an explicit source fact. Do not contradict the premise or replace an existing action. Add concrete, filmable development rather than generic filler. Aim for {remaining_length} non-whitespace characters and keep this continuation between {lower_bound} and {upper_bound} characters. End on a complete sentence. Return only the continuation without headings, labels, markdown, or explanations."""


def enrichment_length_repair_system(strength: int, target_length: int) -> str:
    """Ask for an integrated full rewrite when a valid draft is too short."""
    strength = max(1, min(100, int(strength)))
    target_length = max(100, min(2000, int(target_length)))
    lower_bound = round(target_length * 0.9)
    upper_bound = round(target_length * 1.1)
    return f"""Rewrite the entire SHORT ENRICHED DRAFT as one complete integrated video prompt containing between {lower_bound} and {upper_bound} non-whitespace characters, targeting {target_length}. Use exactly the same language as the ORIGINAL SOURCE. Preserve every source identity, reference, count, action, order, continuity fact, vocalization, and quoted utterance verbatim and exactly once. Expand only inside the supplied action chain: weave concrete, filmable camera, framing, lighting, environment, timing, physical continuity, gesture, texture, and supported sound detail beside the exact existing action it develops. Do not append a continuation to the draft, repeat or restart the source, or add any new event, destination, message, relationship, task, encounter, character, dialogue, aftermath, or sequel. Once the final source action, vocalization, or dialogue is complete, stop immediately. Creative strength is {strength}/100, which controls detail density only and never authorizes a new plot. Return only the complete rewritten prompt without headings, labels, markdown, explanations, or H3 fields."""


def enrichment_segment_system(strength: int, segment_length: int) -> str:
    """Expand one contiguous source-action segment for ordered in-place assembly."""
    strength = max(1, min(100, int(strength)))
    segment_length = max(160, min(600, int(segment_length)))
    lower_bound = round(segment_length * 0.8)
    upper_bound = round(segment_length * 1.2)
    return f"""Enrich only SOURCE ACTION SEGMENT as one contiguous part of a larger video prompt. Target {segment_length} non-whitespace characters and stay between {lower_bound} and {upper_bound}. Preserve every fact, action, reference, vocalization, and quoted utterance in this segment exactly once and in order. Develop only the visible execution of these same actions with concrete camera, framing, lighting, environment, timing, physical continuity, gesture, texture, and supported sound detail. Do not summarize the full source, restart earlier actions, continue beyond this segment, or add a new event, destination, message, relationship, task, encounter, character, dialogue, aftermath, or sequel. Do not add any speech or vocalization not explicitly present in this segment. Creative strength is {strength}/100 and controls detail density only. Use exactly the source language. Return only the enriched segment, with no heading, label, numbering, markdown, explanation, or H3 field."""


def enrichment_segment_detail_system(facet: str, detail_length: int) -> str:
    detail_length = max(100, min(500, int(detail_length)))
    return f"""Write only supplemental {facet} detail for the CURRENT ENRICHED ACTION SEGMENT, about {detail_length} non-whitespace characters. The detail will be inserted inside this same action segment. Do not repeat or paraphrase the source actions, identities, references, vocalizations, or dialogue. Do not advance time, finish the action, add an aftermath, or introduce any new event, destination, message, relationship, task, encounter, character, speech, vocalization, harm, or plot. Add only concrete filmable detail of {facet} that is simultaneous with the existing action. Use exactly the source language. Return only the supplemental prose, no heading, label, markdown, quotation, or explanation."""


def enrichment_review_system(strength: int = 50, target_length: int = 500) -> str:
    strength = max(0, min(100, int(strength)))
    target_length = max(100, min(2000, int(target_length)))
    if strength <= 30:
        allowed = "Only minimal framing, continuity, physical motion, and directly supported ambience additions are allowed."
    elif strength <= 60:
        allowed = "Compatible lighting, atmosphere, environmental texture, camera, sound, and small physical reactions are allowed, but new characters, relationships, dialogue, and plot turns are not."
    elif strength <= 80:
        allowed = "Compatible setting, lighting, atmosphere, camera, sound, gestures, and supporting minor props are allowed, but new characters, relationships, dialogue, and independent plot events are not."
    else:
        allowed = "Creative additions are expected: compatible setting, time, weather, lighting, wardrobe detail, minor props, gestures, camera progression, sound design, and safe, neutral minor events are allowed. Reject contradictions, replacement of the core premise, new principal characters, or changes to supplied identities, counts, relationships, actions, order, references, and dialogue."
    return f"""Review the PROPOSED ENRICHED PROMPT against the ORIGINAL SOURCE. Creative strength is {strength}/100. Target output length is {target_length} characters. The proposal must integrate the complete source exactly once; return FAIL when it prepends, appends, quotes, or repeats the unchanged source, repeats a substantial action block, or restarts the action sequence. {allowed} Every addition must occur within an existing source action. Return FAIL for any new event, destination, message, relationship, task, encounter, aftermath, or story continuation after the final source action, vocalization, or dialogue. Return FAIL if an addition changes the nature of the source activity, its genre or emotional tone. Return FAIL for invented violence, harm, coercion, sexual content, crime, disease, injury, or biography such as names, ages, occupations, histories, body measurements, or medical conditions unless the ORIGINAL SOURCE explicitly contains that fact. Every source utterance must remain verbatim and exactly once; return FAIL for any other dialogue, even when the source contains some dialogue. Do not reject or sanitize any sensitive fact explicitly supplied by the source. Return exactly PASS only when every explicit source fact is preserved without contradiction or omission, additions obey this strength tier, each addition is woven beside the source action it develops, and the output stops at the source ending. Return only PASS or FAIL."""
