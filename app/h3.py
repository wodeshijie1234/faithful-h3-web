...
def enrichment_system(strength: int, target_length: int = 500) -> str:
    ...
    if strength <= 30:
        ...
        creativity = "You may introduce compatible setting, time of day, weather, lighting, wardrobe detail, minor props, gestures, camera progression, sound design, and safe, neutral minor events that develop the requested scene. Creative additions are expected, but they must not contradict, replace, or remove the supplied people, identities, counts, relationships, actions, order, references, or dialogue. Do not add new principal characters or change the core premise."
    return f"""Return one complete enriched video prompt, not additions and not an afterword. Begin with the exact input text verbatim as the opening premise, then continue it directly with the enrichment; this required opening anchor is not a disconnected repetition. Target output length is {target_length} characters; the finished result, including that opening premise, must contain between {lower_bound} and {upper_bound} non-whitespace characters when the source is not already longer. Never truncate, omit, contradict, or sanitize source facts merely to meet the target. Keep every source identity, reference ID, count, position, action, camera change, sequence, continuity condition, and dialogue in the original order. Write in exactly the same language as the input; Chinese input must produce Chinese output. Creative strength is {strength}/100. {creativity} Do not change the nature of the source activity, its genre or emotional tone. Unless already explicit in the source, never invent violence, harm, coercion, sexual content, crime, disease, or injury. Never invent biography such as a person's name, age, occupation, history, body measurements, or medical condition. Do not add dialogue unless the source requests dialogue. These restrictions apply only to new inventions: preserve any such fact that the user explicitly supplied. Integrate all additions into one cohesive production-ready prompt instead of repeating the source and appending an afterword. Use the requested length for concrete, filmable development rather than repetition or generic filler. No headings, labels, markdown, explanations, or H3 fields."""
...
def enrichment_repair_system(strength: int, target_length: int = 500) -> str:
    ...
    if strength <= 30:
        ...
        creativity = "Retain or add compatible setting, time, weather, lighting, wardrobe detail, minor props, gestures, camera progression, sound design, and safe, neutral minor events needed to reach the requested length. Remove contradictions, replacement of the core premise, new principal characters, or changes to supplied identities, counts, relationships, actions, order, references, and dialogue."
    return f"""Repair the PROPOSED ENRICHMENT against the ORIGINAL SOURCE. Return one complete enriched prompt in exactly the same language as the ORIGINAL SOURCE; Chinese source must produce Chinese output. Begin with the ORIGINAL SOURCE verbatim as the opening premise, then continue directly with integrated enrichment; this required opening anchor is not a disconnected repetition. Target output length is {target_length} characters; the finished result, including that opening premise, must contain between {lower_bound} and {upper_bound} non-whitespace characters when the source is not already longer. Preserve every explicit source identity, reference ID, count, position, action, camera direction, sequence, continuity statement, and dialogue in the original order. Creative strength is {strength}/100. {creativity} Do not change the nature of the source activity, its genre or emotional tone. Remove invented violence, harm, coercion, sexual content, crime, disease, or injury unless it is explicit in the source. Remove invented biography, including names, ages, occupations, histories, body measurements, and medical conditions. Remove invented dialogue unless the source requests dialogue. Never remove or sanitize those facts when the ORIGINAL SOURCE explicitly supplies them. Integrate additions into the prompt itself, remove source-plus-afterword formatting, repetition, and generic filler, and never replace a source action with a different action. Return only the repaired prompt, without headings, labels, markdown, or explanations."""
...
def enrichment_continuation_system(strength: int, remaining_length: int) -> str:
    ...
    if strength <= 30:
        ...
        allowed = "You may continue compatible setting, time, weather, lighting, wardrobe detail, minor props, gestures, camera progression, sound design, and safe, neutral minor events."
...
def enrichment_review_system(strength: int = 50, target_length: int = 500) -> str:
    ...
    return f"""Review the PROPOSED ENRICHED PROMPT against the ORIGINAL SOURCE. Creative strength is {strength}/100. Target output length is {target_length} characters. The proposal must begin with the ORIGINAL SOURCE verbatim as its opening premise; treat that required anchor as valid integration, not as disconnected repetition. {allowed} Return FAIL if an addition changes the nature of the source activity, its genre or emotional tone. Return FAIL for invented violence, harm, coercion, sexual content, crime, disease, injury, or biography such as names, ages, occupations, histories, body measurements, or medical conditions unless the ORIGINAL SOURCE explicitly contains that fact. Return FAIL for invented dialogue unless the source requests dialogue. Do not reject or sanitize any sensitive fact explicitly supplied by the source. Return exactly PASS only when every explicit source fact is preserved without contradiction or omission, additions obey this strength tier, and the result is an integrated prompt rather than a disconnected afterword or unrelated repetition. Return only PASS or FAIL."""
